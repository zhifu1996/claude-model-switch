...
- 支持在菜单中直接切换 `ANTHROPIC_BASE_URL` 和 `ANTHROPIC_AUTH_TOKEN`
...
Endpoint:
  Base URL (ANTHROPIC_BASE_URL):      https://cliproxyapi.1049131.xyz
...
| [b] | 设置 Base URL (ANTHROPIC_BASE_URL) |
| [k] | 设置认证 Token (ANTHROPIC_AUTH_TOKEN) |
| [a] | 自定义输入模型名称和配置目标 |
| [r] | 将所有模型同步为当前主模型 |
| [q] | 退出 |
...
| 环境变量 | 说明 |
|----------|------|
| `ANTHROPIC_BASE_URL` | API 基础地址（可通过菜单 `[b]` 设置） |
| `ANTHROPIC_AUTH_TOKEN` | API 认证 Token（可通过菜单 `[k]` 设置） |
| `ANTHROPIC_MODEL` | 主模型 |
